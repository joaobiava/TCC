...
def build_subGraph(G: ig.Graph, edges_by_time, timestamps_only, start_time, end_time):
    left = bisect.bisect_left(timestamps_only, start_time)

    right = bisect.bisect(timestamps_only, end_time)
...
def execute_timeWindow(G, all_users, ts_begin, ts_end, edges_by_time, timestamps_only, alfa):
    ...
    current_time = ts_begin + TIME_WINDOW + (TIME_WINDOW * 4)
    ...
    current_sub = build_subGraph(G, edges_by_time, timestamps_only, 0, current_time)

    while current_time < ts_end:
        future_time = min(current_time + TIME_WINDOW, ts_end)

        future_sub = build_subGraph(G, edges_by_time, timestamps_only, current_time, future_time)

        current_readable = pd.to_datetime(current_time, unit='ms')
        future_readable = pd.to_datetime(future_time, unit='ms')
        log.info("="*60)
        log.info(f"JANELA DE AVALIACAO: {current_readable} ate {future_readable}")
        log.info("="*60)
        ...
        log.info(f"usuarios ativos em ambas as janelas: {len(valid_users)}")
        ...
        for user_index in valid_users:
            ...
            all_ndcgs.append(n)
            window_precision.append(p)
            window_ndcg.append(n)

        mean_precision_window = sum(window_precision) / len(window_precision)
        mean_ndcg_window = sum(window_ndcg) / len(window_ndcg)
        log.info(f"[Resultado da Janela] P@{TOP_KS}: {mean_precision_window} | nDCG@{TOP_KS}: {mean_ndcg_window}")
...
if __name__ == "__main__":
    ...
    graph_name = {1: 'append_users_items_allUsers', 2: 'append_users_sessions_items_allUsers', 3: 'append_regions_users_sessions_items_allUsers', 4: 'append_devices_users_sessions_regions_items_allUsers'}[choice]
